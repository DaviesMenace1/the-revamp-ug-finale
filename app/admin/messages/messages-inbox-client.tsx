...
import { getAdminConversationSummaries, getConversationMessages, markAdminMessagesRead, prepareMessageAttachment, sendAdminReply, updateConversationStatus } from '@/lib/actions/messages'
...
const PAGE_REFERENCES = [
  ...
  { label: 'View consultations', href: '/client/consultations' },
  { label: 'View projects', href: '/client/projects' },
  { label: 'View orders', href: '/client/orders' },
  { label: 'View documents', href: '/client/documents' },
...
export default function MessagesInboxClient({ initialConversations = [], initialLoadError = null }: { initialConversations: ConversationSummary[]; initialLoadError?: string | null }) {
  ...
  async function handleFileSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    event.target.value = ''
    if (!file) return
    if (attachments.filter((attachment) => attachment.kind === 'file').length >= 5) {
  ...
  function handleSend() {
    if (!selectedId || isPending || uploadingAttachment) return
    const body = draft.trim()
    const reference = PAGE_REFERENCES.find((item) => item.href === selectedReference)
    if (!body && attachments.length === 0 && !reference) return
    const sentAttachments = reference ? [...attachments, { kind: 'link' as const, href: reference.href, label: reference.label }] : attachments
    setDraft('')
    setAttachments([])
    ...
    startTransition(async () => {
      const result = await sendAdminReply(selectedId, body, sentAttachments)
  ...
  function handleStatusChange(status: string) {
    ...
    startTransition(async () => {
      ...
      if (result.success) setConversationsList((prev) => prev.map((conversation) => conversation.id === selectedId ? { ...conversation, status } : conversation))
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      <header className="flex flex-col gap-4 border-b border-border/70 pb-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] uppercase tracking-[0.24em] text-primary">Studio communications</p><h1 className="mt-2 font-serif text-4xl font-light text-foreground">Messages</h1><p className="mt-2 text-sm text-muted-foreground">Reply to clients, share a page they can tap, attach documents, and track delivery.</p></div><Button type="button" variant="outline" onClick={() => void refreshConversations()} disabled={refreshing} className="min-h-11 rounded-none self-start sm:self-auto">{refreshing ? <Loader2 className="mr-2 size-4 animate-spin" /> : <RefreshCw className="mr-2 size-4" />}Refresh inbox</Button></header>
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]"><Card className="overflow-hidden p-0"><div className="space-y-3 border-b border-border/70 bg-muted/10 p-4"><div className="relative"><Search className="absolute left-3 top-3 size-4 text-muted-foreground" /><Input aria-label="Search conversations" placeholder="Search clients or subjects…" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="min-h-11 rounded-none pl-10" /></div><select aria-label="Filter conversations by status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)} className="min-h-11 w-full rounded-none border border-input bg-background px-3 text-sm text-foreground"><option value="all">All statuses</option><option value="open">Open</option><option value="resolved">Resolved</option><option value="archived">Archived</option></select></div><div className="max-h-[58vh] overflow-y-auto">{filteredConversations.map((conversation) => <button key={conversation.id} type="button" onClick={() => void openConversation(conversation.id)} className={`flex min-h-[88px] w-full items-start justify-between gap-3 border-b border-border/20 p-4 text-left transition-colors hover:bg-muted/50 ${selectedId === conversation.id ? 'bg-primary/5' : ''}`}><div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate text-sm font-medium text-foreground">{personName(conversation)}</p>{conversation.adminUnreadCount > 0 && <span className="size-2 shrink-0 rounded-full bg-primary" aria-label="Unread" />}</div><p className="mt-1 truncate text-xs text-muted-foreground">{conversation.subject || 'General conversation'}</p><p className="mt-2 text-[11px] text-muted-foreground">{timeLabel(conversation.lastMessageAt)}</p></div>{conversation.adminUnreadCount > 0 && <span className="rounded-full bg-primary px-2 py-0.5 text-[10px] font-medium text-primary-foreground">{conversation.adminUnreadCount}</span>}</button>)}{filteredConversations.length === 0 && <div className="p-8 text-center"><Inbox className="mx-auto size-7 text-gold" /><p className="mt-3 text-sm text-muted-foreground">No matching conversations.</p></div>}</div></Card>

        <Card className="flex min-h-[620px] flex-col overflow-hidden p-0">{!selected ? <div className="flex flex-1 flex-col items-center justify-center px-6 text-center text-muted-foreground"><MessageSquare className="size-9 text-gold" /><p className="mt-4 font-serif text-2xl text-foreground">Select a conversation</p><p className="mt-2 max-w-sm text-sm leading-6">Choose a client message from the inbox to read and reply.</p></div> : <><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/70 p-4 sm:p-5"><div className="min-w-0"><p className="truncate text-sm font-medium text-foreground">{personName(selected)}</p><p className="mt-1 truncate text-xs text-muted-foreground">{selected.clientEmail || 'No email available'} · {selected.subject || 'General conversation'}</p>{lastUpdated && <p className="mt-1 text-[10px] text-muted-foreground">Inbox refreshed {timeLabel(lastUpdated)}</p>}</div><select aria-label="Conversation status" value={selected.status ?? 'open'} onChange={(event) => handleStatusChange(event.target.value)} disabled={isPending} className="min-h-10 rounded border border-muted bg-transparent px-2 py-1 text-xs text-foreground"><option value="open">Open</option><option value="resolved">Resolved</option><option value="archived">Archived</option></select></div>

          <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-6">{loadingMessages && <div className="flex items-center justify-center gap-2 py-8 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" />Loading conversation…</div>}{!loadingMessages && messages.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">No messages in this conversation yet.</p>}{messages.map((message, index) => { const previous = messages[index - 1]; const showDate = !previous || !sameDay(previous.createdAt, message.createdAt); const isAdmin = message.senderType === 'admin'; const status = deliveryLabel(message); return <div key={message.id}>{showDate && <p className="my-2 text-center text-[10px] uppercase tracking-[0.16em] text-muted-foreground">{dateLabel(message.createdAt)}</p>}<div className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[90%] rounded-xl px-4 py-3 text-sm sm:max-w-[75%] ${isAdmin ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground'}`}><div className="flex items-center justify-between gap-4"><p className={`text-[10px] font-semibold uppercase tracking-[0.12em] ${isAdmin ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>{isAdmin ? 'You' : message.senderName || 'Client'}</p><time dateTime={message.createdAt} className={`text-[10px] ${isAdmin ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>{timeLabel(message.createdAt)}</time></div><p className="mt-2 whitespace-pre-wrap leading-6">{message.body}</p>{message.attachments && message.attachments.length > 0 && <div className="mt-3 space-y-2">{message.attachments.map((attachment, attachmentIndex) => attachment.kind === 'link' && attachment.href && attachment.label ? <a key={`${message.id}-link-${attachmentIndex}`} href={attachment.href} className={`flex items-center gap-2 rounded border px-3 py-2 text-xs font-medium ${isAdmin ? 'border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10' : 'border-border text-foreground hover:bg-background'}`}><Link2 className="size-4 shrink-0" />{attachment.label}</a> : attachment.kind === 'file' && attachment.key ? <a key={`${message.id}-file-${attachmentIndex}`} href={attachmentHref(attachment.key)} target="_blank" rel="noreferrer" className={`flex items-center gap-2 rounded border px-3 py-2 text-xs ${isAdmin ? 'border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10' : 'border-border text-foreground hover:bg-background'}`}><FileText className="size-4 shrink-0" /><span className="truncate">{attachment.filename || 'Attachment'}</span></a> : null)}</div>}{isAdmin && <span className="mt-2 flex items-center justify-end gap-1 text-[10px] text-primary-foreground/70">{status === 'Sending' ? <Loader2 className="size-3 animate-spin" /> : status === 'Sent' ? <Check className="size-3" /> : <CheckCheck className="size-3" />}{status}</span>}</div></div></div>})}<div ref={endRef} /></div>

          <div className="border-t border-border/70 bg-muted/10 p-4 sm:p-5"><div className="mb-3 flex flex-wrap gap-2">{QUICK_REPLIES.map((reply) => <button key={reply} type="button" onClick={() => setDraft(reply)} className="min-h-9 rounded-full border border-border/70 px-3 text-left text-xs text-muted-foreground transition-colors hover:border-primary hover:text-primary">{reply}</button>)}</div><div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><select aria-label="Add a page reference" value={selectedReference} onChange={(event) => setSelectedReference(event.target.value)} className="min-h-11 rounded-none border border-input bg-background px-3 text-sm text-foreground"><option value="">Attach a page reference…</option>{PAGE_REFERENCES.map((reference) => <option key={reference.href} value={reference.href}>{reference.label}</option>)}</select>{selectedReference && <span className="flex items-center gap-2 rounded border border-primary/20 bg-primary/5 px-3 text-xs text-primary"><Link2 className="size-4" />Tappable link included</span>}</div>{attachments.length > 0 && <div className="mb-3 flex flex-wrap gap-2">{attachments.map((attachment, index) => <span key={`${attachment.key}-${index}`} className="flex max-w-full items-center gap-2 rounded-full border border-border/70 bg-background px-3 py-1.5 text-xs text-foreground"><FileText className="size-3.5 shrink-0 text-primary" /><span className="max-w-[180px] truncate">{attachment.filename}</span><button type="button" onClick={() => setAttachments((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="rounded-full p-1 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label={`Remove ${attachment.filename}`}><X className="size-3.5" /></button></span>)}</div>}<div className="flex items-end gap-2"><input ref={fileInputRef} type="file" accept="image/*,.pdf,.txt,.doc,.docx,.rtf" onChange={handleFileSelected} className="sr-only" /><Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={uploadingAttachment || attachments.length >= 5} className="min-h-11 min-w-11 rounded-none px-3" aria-label="Attach a file">{uploadingAttachment ? <Loader2 className="size-4 animate-spin" /> : <Paperclip className="size-4" />}</Button><Textarea value={draft} onChange={(event) => setDraft(event.target.value.slice(0, 2000))} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); handleSend() } }} placeholder="Write a reply… (Enter to send)" rows={3} className="min-h-20 resize-none rounded-none bg-background" aria-label="Reply message" /><Button onClick={handleSend} disabled={isPending || uploadingAttachment || (!draft.trim() && attachments.length === 0 && !selectedReference)} className="min-h-11 rounded-none" aria-label="Send reply">{isPending ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />}</Button></div><p className="mt-2 text-right text-[11px] text-muted-foreground">{draft.length}/2000 · Attachments up to 15 MB · Shift + Enter for a new line</p></div></>}
        </Card>
      </div>
    </div>
  )
