...
export function AccountOverview({ data }: { data: NonNullable<AccountData> }) {
  ...
  return (
    <div className="bg-background">
      ...
      <section className="relative overflow-hidden bg-canvas px-5 pb-8 pt-10 text-obsidian sm:px-8 sm:pb-10 lg:px-12 lg:pt-16">
        ...
        <div className="relative mx-auto max-w-[1440px]">
          <div className="flex items-center gap-5 sm:gap-7">
            ...
            <div className="min-w-0"><p className="font-serif text-lg text-obsidian/75">Welcome back,</p><h1 className="mt-1 truncate font-serif text-4xl font-light leading-none sm:text-5xl">{firstName}</h1><span className="mt-3 inline-flex rounded border border-gilded/70 px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-gilded">{membership?.status === 'active' ? `${membership.membershipType} member` : 'Signature member'}</span></div>
          </div>
          <div className="mt-8 grid gap-px overflow-hidden rounded-lg border border-border bg-white/10 sm:grid-cols-[0.8fr_1.2fr]">
            <div className="bg-muted/50 p-5 sm:p-6"><p className="text-[10px] uppercase tracking-[0.2em] text-obsidian/65">Loyalty points</p><p className="mt-2 font-serif text-4xl text-obsidian sm:text-5xl">{balance.toLocaleString('en-UG')}</p><p className="mt-1 text-sm text-gilded">points</p></div>
            <div className="bg-muted/50 p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-obsidian/85">{loyalty?.nextTier ? `You are ${nextTier.toLocaleString('en-UG')} points away` : 'You have reached our highest tier'}</p><p className="mt-1 text-sm text-obsidian/60">from {loyalty?.nextTier || loyalty?.tier || 'Signature'} tier</p></div><span className="text-xs text-obsidian/55">{balance.toLocaleString('en-UG')} / {(balance + nextTier).toLocaleString('en-UG')}</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-obsidian/10"><div className="h-full rounded-full bg-gilded-light" style={{ width: `${progress}%` }} /></div><Link href="/membership" className="mt-5 flex min-h-11 items-center justify-between rounded border border-border px-4 text-sm text-obsidian/85 transition hover:border-gilded hover:text-gilded">View membership benefits <ArrowRight className="size-4" aria-hidden="true" /></Link></div>
          </div>
        </div>
      </section>
...
function QuickAccess({ href, icon: Icon, label, badge }: { href: string; icon: typeof Package; label: string; badge?: number }) {
  return <Link href={href} className="relative flex min-h-28 flex-col items-center justify-center gap-3 rounded-lg border border-border bg-card px-2 text-center transition hover:border-gilded hover:bg-muted/40"><Icon className="size-6 text-foreground" strokeWidth={1.4} aria-hidden="true" /><span className="text-xs text-foreground/80">{label}</span>{badge ? <span className="absolute right-2 top-2 flex size-5 items-center justify-center rounded-full bg-gilded text-[10px] text-obsidian">{badge}</span> : null}</Link>
}
