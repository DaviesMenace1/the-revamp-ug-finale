...
const EMPTY_FORM: EventForm = { title: '', description: '', image: '', location: '', eventDate: '', capacity: '', membershipTier: 'all', status: 'draft' }
...
export default function EventsAdminClient({ events: initialEvents }: { events: EventRecord[] }) {
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      const event = { ...result.event, eventDate: result.event.eventDate.toISOString(), createdAt: result.event.createdAt.toISOString(), updatedAt: result.event.updatedAt.toISOString() }
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
        <Card className="border-border/20 p-5 sm:p-7">
          ...
          <div className="mt-6 grid gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2"><Field label="Date and time" type="datetime-local" value={form.eventDate} onChange={(value) => setField('eventDate', value)} /><Field label="Location or meeting link" value={form.location} onChange={(value) => setField('location', value)} placeholder="Kyanja studio / online" /></div>
