...
import { getLoyaltyRules } from '@/lib/loyalty/service'
...
export default async function MembershipProgramPage() {
      ...
      <main className="min-h-screen bg-background text-foreground">
        <section className="relative overflow-hidden bg-obsidian px-5 pb-20 pt-36 text-ivory sm:px-8 md:pb-28 md:pt-48 lg:px-16">
          ...
          <div className="relative mx-auto max-w-[1440px]">
            ...
            <div className="mt-10 grid gap-8 border-t border-ivory/15 pt-8 md:grid-cols-[1fr_auto] md:items-end"><p className="max-w-2xl text-base leading-8 text-ivory/70 sm:text-lg">Membership is an open Revamp relationship, not a paid subscription. Your privileges grow through the points you accumulate with the studio.</p><Link href="/account" className="inline-flex min-h-11 items-center gap-2 text-xs uppercase tracking-[0.18em] text-gold underline-offset-4 hover:underline">Explore your account <ArrowRight className="size-4" /></Link></div>
          </div>
        </section>

        <section className="px-5 py-16 sm:px-8 md:py-24 lg:px-16"><div className="mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24"><div><p className="text-[10px] uppercase tracking-[0.28em] text-primary">The membership direction</p><h2 className="mt-5 max-w-xl font-serif text-4xl font-light leading-tight sm:text-6xl">Not just a discount. A design relationship.</h2></div><div className="space-y-6 text-base leading-8 text-muted-foreground sm:text-lg"><p>The programme connects the things that make the Revamp experience feel distinct: access to collections, a closer view of the studio’s point of view, private gatherings, and support for the spaces members are shaping.</p><p>There is no recurring membership fee. Qualifying activity earns points, and the studio manages the thresholds and privileges transparently.</p><Button asChild className="min-h-12 rounded-none px-6 text-xs uppercase tracking-[0.18em]"><Link href="/sign-in?redirect_url=%2Fmembership">View your points <ArrowRight className="ml-2 size-4" /></Link></Button></div></div></section>
        ...
        <section className="px-5 py-16 sm:px-8 md:py-24 lg:px-16"><div className="mx-auto max-w-[1440px]"><div className="mb-10 max-w-2xl"><p className="text-[10px] uppercase tracking-[0.28em] text-primary">Points and privilege</p><h2 className="mt-4 font-serif text-4xl font-light sm:text-6xl">Find your place in the world.</h2></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{rules.tiers.map((tier, index) => <article key={tier.key} className="border border-border p-6 sm:p-8"><span className="text-[10px] uppercase tracking-[0.2em] text-gold">From {tier.lifetimePoints.toLocaleString('en-UG')} points</span><h3 className="mt-12 font-serif text-3xl font-light">{tier.label}</h3><p className="mt-3 text-sm leading-7 text-muted-foreground">{tier.privileges.slice(0, 2).join(' · ') || 'Access to a more connected Revamp experience.'}</p></article>)}</div></div></section>
