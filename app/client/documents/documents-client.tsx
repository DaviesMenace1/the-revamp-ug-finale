        ...
        <div className="grid gap-3">
          ...
          {documents.map((doc) => (
            <Card key={doc.id} className="flex flex-col gap-4 rounded-xl border-border/70 bg-card p-5 shadow-soft transition-shadow hover:shadow-lift sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2"><a href={doc.fileUrl} target="_blank" rel="noreferrer" aria-label={`View ${doc.name}`} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded border border-border px-4 text-xs font-semibold uppercase tracking-[0.14em] text-foreground hover:border-gold hover:text-primary"><FileText className="size-4" /> View <ArrowUpRight className="size-4" /></a><a href={`/api/documents/download?url=${encodeURIComponent(doc.fileUrl)}`} download aria-label={`Download ${doc.name} to device`} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded border border-border px-4 text-xs font-semibold uppercase tracking-[0.14em] text-foreground hover:border-gold hover:text-primary"><Download className="size-4" /> Download</a></div>
            </Card>
          ))}
