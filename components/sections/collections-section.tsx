    ...
    <section className="section-pad bg-background">
      <div className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-16">
        <div className="mb-10 flex flex-col gap-6 border-b border-border/70 pb-7 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.28em] text-primary">The current edit</p>
            <h2 className="mt-3 font-serif text-4xl font-light leading-tight sm:text-6xl">Objects with presence.</h2>
            <p className="mt-4 max-w-md text-sm leading-6 text-muted-foreground">Furniture, lighting, and accents chosen for rooms that are meant to be lived in.</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4 lg:gap-6">
          {items.map((item, index) => <ProductCard key={item.id} product={item} style={{ animationDelay: `${Math.min(index, 8) * 35}ms` }} className="motion-reveal" />)}
        </div>
      </div>
    </section>
  )
