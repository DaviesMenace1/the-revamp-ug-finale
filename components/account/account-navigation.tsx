  ...
  return (
    <>
      ...
      <div className="lg:hidden">
        <button type="button" aria-label="Open account menu" aria-expanded={open} onClick={() => setOpen(true)} className="fixed bottom-5 right-5 z-40 flex size-14 items-center justify-center rounded-full bg-foreground text-background shadow-2xl transition-transform active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
        ...
        {open && <div className="fixed inset-0 z-50 bg-black/45" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false) }}>
          <aside className="absolute right-0 top-0 flex h-full w-[min(88vw,22rem)] flex-col overflow-y-auto bg-background p-5 shadow-2xl" role="dialog" aria-modal="true" aria-label="Account menu">
