          ...
          <form onSubmit={handleSubmit} className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="space-y-10">
              ...
              <div className="rounded-xl border border-border/70 bg-card p-5 motion-reveal" style={{ animationDelay: '180ms' }}>
                ...
                <div className="mt-5 space-y-2 border-y border-border/70 py-4 text-sm"><div className="flex justify-between gap-4 text-muted-foreground"><span>Consultation fee</span><span>{money(appliedQuote?.baseFee ?? pricing.baseFee, appliedQuote?.currency || pricing.currency)}</span></div>{appliedQuote && <div className="flex justify-between gap-4 text-primary"><span>{appliedQuote.promoName || appliedQuote.promoCode || 'Promotion'}</span><span>-{money(appliedQuote.discount, appliedQuote.currency)}</span></div>}<div className="flex justify-between gap-4 text-muted-foreground"><span>{pricing.taxRate > 0 ? (pricing.taxInclusive ? `Includes ${pricing.taxRate}% tax` : `Tax ${pricing.taxRate}%`) : 'Tax rate to be updated in admin'}</span><span>{pricing.taxRate > 0 ? money(appliedQuote?.tax ?? pricing.baseFee * pricing.taxRate / (100 + pricing.taxRate), appliedQuote?.currency || pricing.currency) : 'Set in admin'}</span></div></div>
