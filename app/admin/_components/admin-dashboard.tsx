  ...
  return (
    <div className="min-h-screen bg-muted/30 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
      <header className="mx-auto flex max-w-[1500px] flex-col gap-6 border-b border-border/70 pb-8 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="text-[10px] uppercase tracking-[0.3em] text-primary">The Revamp operations · 01</p><h1 className="mt-3 font-serif text-5xl leading-none text-foreground sm:text-6xl">Dashboard</h1><p className="mt-4 max-w-xl text-sm leading-7 text-muted-foreground">A live view of the studio’s commercial pulse, project work, and latest client activity.</p></div>
        <div className="flex flex-wrap gap-2"><Link href="/admin/orders" className="inline-flex min-h-11 items-center gap-2 rounded border border-border bg-background px-4 text-xs font-semibold uppercase tracking-[0.14em] text-foreground hover:border-gold">Orders <ArrowUpRight className="size-4" /></Link><Link href="/admin/client-projects" className="inline-flex min-h-11 items-center gap-2 rounded bg-foreground px-4 text-xs font-semibold uppercase tracking-[0.14em] text-background hover:bg-gold hover:text-obsidian">Projects <ArrowUpRight className="size-4" /></Link></div>
      </header>
