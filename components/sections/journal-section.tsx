...
export function JournalSection() {
  ...
  // Rotate featured article position every 4 seconds
  useEffect(() => {
  ...
  // Derive current position 1 (featured), position 2 (side top), position 3 (side bottom)
  const featured = articles[activeIndex]
  const rest1 = articles[(activeIndex + 1) % articles.length]
  const rest2 = articles[(activeIndex + 2) % articles.length]
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-border">
          ...
          <Link 
            ...
            className="group relative bg-background overflow-hidden block lg:row-span-2 min-h-[420px] transition-all duration-700"
          >
            <div
              className="aspect-[16/10] lg:aspect-auto lg:h-full min-h-[420px] bg-cover bg-center transition-transform duration-700 group-hover:scale-105"
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 transition-opacity duration-500">
              ...
              <p className="font-sans text-white/70 text-sm leading-relaxed mb-4 max-w-md line-clamp-2">
                {featured.excerpt}
              </p>
              <div className="flex items-center gap-3 font-sans text-xs text-white/50">
          ...
          <div className="flex flex-col gap-px bg-border">
            {[rest1, rest2].map((article) => (
              <Link
                key={`rest-${article.slug}`}
                href={`/journal/${article.slug}`}
                className="group bg-background flex gap-6 p-6 hover:bg-muted/30 transition-all duration-500"
              >
                <div
                  className="flex-none w-28 h-28 bg-cover bg-center transition-transform duration-500 group-hover:scale-105"
