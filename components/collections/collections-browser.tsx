...
export function CollectionsBrowser() {
  ...
  const quick = (searchParams.get('quick') as QuickTag) || 'all'
  const space = (searchParams.get('space') as Space) || 'all'
  const item = (searchParams.get('item') as ItemType) || 'all'
  ...
  const handleFilterChange = (key: string, value: string) => {
    ...
    router.push(queryString ? `?${queryString}` : '/collections', { scroll: false })
  }
      ...
      <aside className="space-y-10">
        ...
        <FilterGroup title="Quick Filters">
          <div className="flex flex-wrap lg:flex-col gap-2">
            {QUICK_FILTERS.map((f) => (
              <button
                key={f.value}
                onClick={() => handleFilterChange('quick', f.value)}
                className={cn(
        ...
        <FilterGroup title="By Category">
          <FilterList
            options={SPACES}
            active={space}
            onSelect={(v) => handleFilterChange('space', v)}
          />
        </FilterGroup>
        ...
        <FilterGroup title="By Item">
          <FilterList
            options={ITEM_TYPES}
            active={item}
            onSelect={(v) => handleFilterChange('item', v)}
          />
        </FilterGroup>
      </aside>
