...
const MODE_META: Record<string, { label: string; icon: LucideIcon; detail: string }> = {
  ...
  on_site: { label: 'On-site consultation', icon: MapPin, detail: 'Meet with the studio at the address you provide.' },
  in_person: { label: 'On-site consultation', icon: MapPin, detail: 'Meet with the studio at the address you provide.' },
...
export default function BookConsultationClient({ slots = [], loadError = null }: { slots: Slot[]; loadError?: string | null }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if ((selectedSlot.mode === 'on_site' || selectedSlot.mode === 'in_person') && !siteAddress.trim()) {
      setError('Please add the address where the on-site consultation will take place.')
      return
    }
    setIsSubmitting(true)
    try {
      ...
      <>
        ...
        <main className="flex min-h-[75vh] items-center justify-center bg-background px-5 py-20 sm:px-8">
          <div className="motion-reveal w-full max-w-2xl rounded-2xl border border-border/70 bg-card p-8 text-center shadow-lift sm:p-12">
            ...
            <Button type="button" onClick={() => router.push('/client/consultations')} className="mt-7 min-h-11 rounded-none px-6 text-xs uppercase tracking-[0.16em]">See my consultation details</Button>
          </div>
        </main>
        <SiteFooter />
        ...
        <section className="px-5 py-12 sm:px-8 md:py-20 lg:px-16">
          <form onSubmit={handleSubmit} className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="space-y-10">
              ...
              {slots.length === 0 ? <Button type="button" onClick={() => router.push('/contact')} className="motion-reveal min-h-12 w-full rounded-none text-xs uppercase tracking-[0.18em]" style={{ animationDelay: '220ms' }}>Ask the studio for an available time</Button> : <Button type="submit" disabled={isSubmitting} className="motion-reveal min-h-12 w-full rounded-none text-xs uppercase tracking-[0.18em]" style={{ animationDelay: '220ms' }}>{isSubmitting ? `Preparing ${money(totalAmount, appliedQuote?.currency || pricing.currency)} payment…` : `Confirm my time and pay ${money(totalAmount, appliedQuote?.currency || pricing.currency)}`}</Button>}
