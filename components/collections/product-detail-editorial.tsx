...
import { ArrowRight, Check, ChevronDown, Heart, Ruler, Send, Share2, ShoppingCart, ShieldCheck, Sparkle, Truck } from '@/components/ui/luxury-icons'
  ...
  return <div className="space-y-10 lg:space-y-16">
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(22rem,0.9fr)] lg:gap-14">
      ...
      <section className="min-w-0 lg:pt-2"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[0.24em] text-primary">{product?.brand || category}</p><p className="mt-2 text-xs text-muted-foreground">{category}</p></div><button type="button" onClick={() => setShareOpen(true)} className="inline-flex min-h-10 items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-muted-foreground hover:text-foreground"><Share2 className="size-4" /> Share</button></div><h1 className="mt-4 font-serif text-5xl font-light leading-[0.92] text-foreground sm:text-6xl">{product?.name || 'Untitled piece'}</h1><div className="mt-5 flex flex-wrap items-baseline gap-3"><div><div className="flex flex-wrap items-baseline gap-2"><span className="font-serif text-2xl text-foreground">{formatMoney(totalPrice, currency)}</span>{tradeOriginalPrice > basePrice && <span className="text-sm text-muted-foreground line-through">{formatMoney(tradeOriginalPrice * quantity, currency)}</span>}</div>{tradeOriginalPrice > basePrice && <p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-700">Trade member price</p>}</div>{reviewsCount > 0 && <span className="flex items-center gap-1 text-xs text-muted-foreground"><span className="text-gold">{'★'.repeat(Math.round(rating || 5))}</span> ({reviewsCount} reviews)</span>}</div><p className="mt-5 max-w-xl text-sm leading-7 text-muted-foreground">{product?.description || 'A carefully selected piece with considered proportions, material, and character.'}</p>
        ...
        <div className="mt-6 grid grid-cols-3 gap-3 border-y border-border/70 py-5 text-center text-[10px] text-muted-foreground"><div><Truck className="mx-auto mb-2 size-5 text-primary" />Worldwide sourcing</div><div><ShieldCheck className="mx-auto mb-2 size-5 text-primary" />Secure payments</div><div><ShoppingCart className="mx-auto mb-2 size-5 text-primary" />White glove delivery</div></div>
