...
function ApproachSection() {
  ...
  return (
    <section className="border-y border-foreground/10 bg-canvas px-5 py-20 sm:px-8 sm:py-28 lg:px-12 lg:py-32"><div className="mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:gap-20"><div><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/65">03 — The Revamp approach</p><h2 className="mt-5 max-w-md font-serif text-5xl font-light leading-[0.9] sm:text-7xl">From vision<br /><span className="italic text-foreground/60">to reality.</span></h2><p className="mt-7 max-w-sm text-sm leading-7 text-foreground/60">We are not simply a designer who hands over drawings. We stay with the work until the final detail is in place.</p></div><div className="border-t border-foreground/15">{steps.map(([number, title, description]) => <div key={number} className="grid grid-cols-[2.5rem_0.75fr_1fr] gap-3 border-b border-foreground/15 py-5 sm:grid-cols-[3rem_0.65fr_1fr] sm:gap-6"><span className="text-[10px] tracking-[0.18em] text-foreground/60">{number}</span><h3 className="font-serif text-2xl font-light leading-none sm:text-3xl">{title}</h3><p className="text-xs leading-5 text-foreground/65 sm:text-sm sm:leading-6">{description}</p></div>)}</div></div></section>
  )
}
...
function MaterialSection() {
  ...
  return (
    <section className="bg-background px-5 py-20 sm:px-8 sm:py-28 lg:px-12 lg:py-36"><div className="mx-auto max-w-[1440px]"><div className="mb-10 max-w-3xl"><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/65">04 — Material dialogue</p><h2 className="mt-4 font-serif text-5xl font-light leading-[0.9] sm:text-7xl">Material is architecture.</h2></div><div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-4 md:items-start">{materials.map(([name, description], index) => <div key={name} className={cn('relative min-h-48 overflow-hidden border border-foreground/15 bg-canvas dark:bg-background px-4 py-5 sm:min-h-64 sm:px-6 sm:py-7', index % 2 === 1 && 'mt-8 md:mt-12')}><div className="absolute inset-0 bg-gradient-to-br from-canvas-dark via-background to-canvas opacity-90" /><div className="relative flex h-full min-h-40 flex-col justify-between sm:min-h-56"><div className="flex items-start justify-between"><span className="text-[9px] uppercase tracking-[0.18em] text-foreground/60">0{index + 1}</span><span className="size-2 rounded-full bg-gold/70" /></div><div><h3 className="font-serif text-3xl font-light leading-none sm:text-4xl">{name}</h3><p className="mt-3 max-w-[11rem] text-[10px] leading-4 text-foreground/65">{description}</p></div></div></div>)}</div></div></section>
  )
}
  ...
  return (
    <>
      <main className="min-h-screen bg-background">
        <section className="relative overflow-hidden bg-canvas dark:bg-background px-5 pb-16 pt-28 sm:px-8 sm:pb-24 sm:pt-36 lg:px-12 lg:pb-32"><div className="mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[1fr_0.62fr] lg:items-end"><div><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/65">The Revamp House</p><h1 className="mt-5 max-w-5xl font-serif text-[clamp(3.8rem,9vw,8rem)] font-light leading-[0.82] tracking-[-0.04em]">A complete<br /><span className="italic text-foreground/60">design house.</span></h1></div><div className="max-w-sm border-t border-foreground/20 pt-6 lg:mb-1"><p className="text-sm leading-7 text-foreground/75 sm:text-base">Architecture, interiors and objects — considered together.</p><div className="mt-7"><EditorialLink href="/book-consultation">Begin a conversation</EditorialLink></div></div></div><div className="pointer-events-none absolute -bottom-14 right-[8%] hidden font-serif text-[17rem] font-light leading-none text-foreground/[0.035] lg:block">01</div></section>
        ...
        <section className="bg-background px-5 py-20 sm:px-8 sm:py-28 lg:px-12 lg:py-36"><div className="mx-auto max-w-[1440px]"><div className="mb-10 grid gap-8 border-b border-foreground/15 pb-8 md:grid-cols-[0.65fr_1.35fr] md:items-end"><div><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/65">01 — The services</p><h2 className="mt-4 max-w-lg font-serif text-5xl font-light leading-[0.9] sm:text-7xl">Capabilities<br /><span className="italic text-foreground/60">with depth.</span></h2></div><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between sm:gap-8"><p className="max-w-lg text-sm leading-7 text-foreground/60">Each discipline is part of the same composition. Explore the chapters that make up the Revamp practice.</p><div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-foreground/50"><span>{totalResults}</span> {totalResults === 1 ? 'service' : 'services'}</div></div></div>
