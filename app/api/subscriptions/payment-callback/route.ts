...
export async function GET(request: NextRequest) {
  ...
  if (!resolvedReference && chargeId) {
    try {
      ...
      const chargeProgram = String(charge?.reference || '').toUpperCase().includes('REV-SUB-TRADE-') ? 'trade' : null
  ...
  if (status === 'cancelled' || status === 'failed') {
    ...
    return redirectToProgram(request, { program, payment: 'failed', message: 'The subscription payment was cancelled or failed.' })
  }
  if (!chargeId) return redirectToProgram(request, { program, payment: 'pending', message: 'The subscription payment is still awaiting authorization.' })

  try {
    const result = await settleSubscriptionPayment({ transactionReference: resolvedReference, chargeId })
    if (result.success) return redirectToProgram(request, { program, payment: 'success', subscriptionId: result.subscriptionId })
    if (result.status === 'pending') return redirectToProgram(request, { program, payment: 'pending', message: result.error || 'The subscription payment is still awaiting authorization.' })
    return redirectToProgram(request, { program, payment: 'failed', message: result.error || 'The subscription payment could not be verified.' })
  } catch (error) {
