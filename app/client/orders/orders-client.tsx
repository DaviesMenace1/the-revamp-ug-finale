        ...
        {selectedOrder && (
          <section aria-label={`Tracking details for ${selectedOrder.orderNumber}`} className="scroll-mt-6 rounded-xl border border-primary/40 bg-card p-5 shadow-lift sm:p-7">
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              ...
              <div><h3 className="flex items-center gap-2 font-serif text-2xl text-foreground"><MapPin className="size-4 text-primary" /> {selectedDelivery?.deliveryMethod === 'pickup_station' ? 'Pickup station' : 'Delivery details'}</h3>{selectedPickupStation ? <div className="mt-3 space-y-2 text-sm text-muted-foreground"><p className="font-medium text-foreground">{typeof selectedPickupStation.name === 'string' ? selectedPickupStation.name : 'Pickup station'}</p><p>{typeof selectedPickupStation.address === 'string' ? selectedPickupStation.address : ''}</p><p>{typeof selectedPickupStation.city === 'string' ? selectedPickupStation.city : ''}{typeof selectedPickupStation.region === 'string' && selectedPickupStation.region ? ` · ${selectedPickupStation.region}` : ''}</p>{typeof selectedPickupStation.instructions === 'string' && selectedPickupStation.instructions && <p className="text-xs text-primary">{selectedPickupStation.instructions}</p>}</div> : selectedAddress.length > 0 ? <div className="mt-3 space-y-2 text-sm text-muted-foreground">{selectedAddress.map((entry) => <p key={entry.key}>{entry.value}</p>)}</div> : <p className="mt-3 text-sm text-muted-foreground">Delivery details are not available for this order.</p>}<p className="mt-5 flex items-center gap-2 text-xs text-muted-foreground"><CalendarDays className="size-3" />Last updated {dateLabel(selectedOrder.updatedAt)}</p></div>
            </div>
          </section>
        )}
