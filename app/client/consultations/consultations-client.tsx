...
import { ArrowRight, Building2, Calendar, Clock3, MapPin, Video } from 'lucide-react'
  ...
  return (
    <PortalLayout portalName="Client Portal" portalSlug="client" navItems={clientNavItems}>
      <div className="space-y-8 pb-8">
        <header className="relative overflow-hidden rounded-2xl bg-foreground px-6 py-8 text-background shadow-lift sm:px-10 sm:py-10"><div className="absolute -right-20 -top-24 size-64 rounded-full border border-gold/25" /><div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] uppercase tracking-[0.28em] text-gold">Your studio calendar</p><h1 className="mt-4 font-serif text-4xl sm:text-6xl">Consultations</h1><p className="mt-4 max-w-xl text-sm leading-7 text-background/70">Keep the next conversation, meeting format, and project brief in one place.</p></div><Link href="/book-consultation" className="inline-flex min-h-11 items-center justify-center gap-2 rounded border border-gold/50 px-4 text-xs font-semibold uppercase tracking-[0.14em] text-gold hover:bg-gold hover:text-foreground">Book another conversation <ArrowRight className="size-4" /></Link></div></header>
        ...
        <section className="grid gap-4 sm:grid-cols-3"><Card className="rounded-xl border-border/70 bg-card p-5 shadow-soft"><p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Total conversations</p><p className="mt-4 font-serif text-4xl text-foreground">{consultations.length}</p></Card><Card className="rounded-xl border-border/70 bg-card p-5 shadow-soft"><p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Upcoming</p><p className="mt-4 font-serif text-4xl text-foreground">{consultations.filter((consultation) => consultation.preferredDate && new Date(consultation.preferredDate) > new Date() && consultation.status !== 'cancelled').length}</p></Card><Card className="rounded-xl border-border/70 bg-card p-5 shadow-soft"><p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Next step</p><p className="mt-4 text-sm leading-6 text-muted-foreground">Bring your space, references, and the question you want to solve.</p></Card></section>

        <div className="grid gap-4">{consultations.map((consultation) => { const meta = modeMeta(consultation.mode); const ModeIcon = meta.icon; return <Card key={consultation.id} className="rounded-xl border-border/70 bg-card p-6 shadow-soft sm:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[10px] uppercase tracking-[0.2em] text-primary">Consultation brief</p><h2 className="mt-2 font-serif text-3xl font-light text-foreground">{consultation.title}</h2><p className="mt-2 text-sm capitalize text-muted-foreground">{consultation.serviceType?.replaceAll('_', ' ') || 'Design conversation'}</p></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-medium capitalize ${STATUS_COLORS[consultation.status ?? 'pending'] || STATUS_COLORS.pending}`}>{consultation.status ?? 'pending'}</span></div>{consultation.description && <p className="mt-6 max-w-3xl text-sm leading-7 text-muted-foreground">{consultation.description}</p>}<div className="mt-6 grid gap-3 border-y border-border/70 py-5 sm:grid-cols-3"><div className="flex items-start gap-3"><Calendar className="mt-0.5 size-4 text-gold" /><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">When</p><p className="mt-1 text-sm text-foreground">{consultation.preferredDate ? `${dateLabel(consultation.preferredDate)} · ${timeLabel(consultation.preferredDate)}` : 'The studio will confirm a time'}</p></div></div><div className="flex items-start gap-3"><ModeIcon className="mt-0.5 size-4 text-gold" /><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Format</p><p className="mt-1 text-sm text-foreground">{meta.label}</p></div></div><div className="flex items-start gap-3"><Clock3 className="mt-0.5 size-4 text-gold" /><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Duration</p><p className="mt-1 text-sm text-foreground">{consultation.durationMinutes} minutes</p></div></div></div>{consultation.location && <p className="mt-5 text-sm text-muted-foreground">Location: <span className="text-foreground">{consultation.location}</span></p>}{consultation.meetingLink && <a href={consultation.meetingLink} target="_blank" rel="noreferrer" className="mt-5 inline-flex min-h-11 items-center gap-2 rounded border border-border px-4 text-xs font-semibold uppercase tracking-[0.14em] text-foreground hover:border-gold hover:text-primary">Open meeting link <ArrowRight className="size-4" /></a>}</Card> })}{consultations.length === 0 && <div className="flex flex-col items-center rounded-xl border border-dashed border-border/70 p-12 text-center"><Calendar className="size-8 text-gold" /><p className="mt-4 font-serif text-2xl text-foreground">No consultations booked yet.</p><p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">Choose an open time to start a design conversation, or send the studio a brief if no times are currently visible.</p><div className="mt-6 flex flex-wrap justify-center gap-2"><Link href="/book-consultation" className="inline-flex min-h-11 items-center gap-2 rounded bg-foreground px-4 text-xs font-semibold uppercase tracking-[0.14em] text-background">View availability <ArrowRight className="size-4" /></Link><Link href="/contact" className="inline-flex min-h-11 items-center rounded border border-border px-4 text-xs font-semibold uppercase tracking-[0.14em] text-foreground">Send a brief</Link></div></div>}</div>
      </div>
    </PortalLayout>
  )
}
