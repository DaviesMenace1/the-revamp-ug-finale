        ...
        <div className="flex flex-col gap-3 mt-10">
          <div className="flex items-center justify-between border border-border px-4 py-2">
            ...
            <div className="flex items-center gap-4">
              <button type="button" onClick={() => setQuantity((v) => Math.max(1, v - 1))} aria-label="Decrease quantity">−</button>
              <span className="min-w-4 text-center font-sans text-sm">{quantity}</span>
              <button type="button" onClick={() => setQuantity((v) => Math.min(99, v + 1))} aria-label="Increase quantity">+</button>
            </div>
          </div>
          <Button
            size="lg"
            onClick={() => {
              addToCart(
                product,
                quantity,
                product.colors.find((item) => item.label === color),
                product.fabrics.find((item) => item.label === fabric),
                product.addons.filter((item) => addons.includes(item.label))
              )
              setAdded(true)
