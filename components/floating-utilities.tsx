...
export function FloatingUtilities() {
  ...
  return (
    <>
      ...
      <button type="button" onClick={toggleSupport} aria-label={supportOpen ? 'Close support and contact options' : 'Open support and contact options'} aria-expanded={supportOpen} className="fixed bottom-5 right-5 z-[70] flex size-14 items-center justify-center rounded-full bg-obsidian text-canvas shadow-[0_14px_35px_rgba(28,28,28,0.24)] ring-2 ring-canvas transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gilded focus-visible:ring-offset-2 sm:right-8"><span className="transition-transform duration-200">{supportOpen ? <X className="size-5" aria-hidden="true" /> : <Headphones className="size-5" aria-hidden="true" />}</span></button>
...
function FloatingAction({ label, href, onClick, icon, tone = 'default' }: FloatingActionProps) {
  const className = `flex min-h-12 w-full items-center gap-3 rounded-md border bg-canvas/95 px-4 text-left text-xs uppercase tracking-[0.14em] text-obsidian shadow-lg backdrop-blur-xl transition-transform hover:-translate-y-0.5 ${tone === 'whatsapp' ? 'border-[#25D366]/40' : tone === 'instagram' ? 'border-[#C13584]/40' : 'border-obsidian/15'}`
