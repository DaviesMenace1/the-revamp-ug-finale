...
function ServiceChapter({ service, category, index }: { service: ServiceItem; category: ServiceCategory; index: number }) {
  ...
  if (variant === 1) {
    return (
      <article className="grid gap-7 border-t border-foreground/15 py-10 md:grid-cols-[0.7fr_1fr] md:items-center md:gap-16 md:py-16">
        <div className="order-2 md:order-1 md:pl-[12%]"><span className="text-[10px] tracking-[0.22em] text-foreground/45">{String(index + 1).padStart(2, '0')}</span><h3 className="mt-5 max-w-md font-serif text-4xl font-light leading-[0.9] sm:text-6xl">{service.name}</h3><p className="mt-5 max-w-sm text-sm leading-7 text-foreground/60">{service.description || `A considered ${service.name.toLowerCase()} service shaped around the architecture and the life within it.`}</p><div className="mt-7"><EditorialLink href={detailHref}>Explore {service.name}</EditorialLink></div></div>
  ...
  if (variant === 2) {
    return (
      <article className="border-t border-foreground/15 py-10 md:py-16"><div className="grid gap-4 sm:grid-cols-[1.1fr_0.9fr] sm:items-end"><Link href={detailHref} className="group block"><div className="relative aspect-[5/4] overflow-hidden bg-canvas-dark"><img src={image} alt={service.name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" /><span className="absolute left-4 top-4 text-[10px] tracking-[0.22em] text-white/75">{String(index + 1).padStart(2, '0')}</span></div></Link><div className="px-1 pb-1 sm:pl-8"><p className="text-[10px] uppercase tracking-[0.2em] text-foreground/45">{category.name}</p><h3 className="mt-3 font-serif text-4xl font-light leading-[0.9] sm:text-5xl">{service.name}</h3><p className="mt-4 max-w-sm text-sm leading-6 text-foreground/60">{service.description || 'Material, proportion and atmosphere held in balance.'}</p><div className="mt-6"><EditorialLink href={detailHref}>Explore service</EditorialLink></div></div></div></article>
    )
  }

  return (
    <article className={cn('group border-t border-foreground/15 py-10 md:py-16', variant === 3 && 'md:pr-[12%]')}>
      <Link href={detailHref} className="grid gap-6 md:grid-cols-[0.72fr_1.28fr] md:items-center md:gap-16"><div className="order-2 md:order-1"><span className="text-[10px] tracking-[0.22em] text-foreground/45">{String(index + 1).padStart(2, '0')}</span><h3 className="mt-4 font-serif text-4xl font-light leading-[0.9] sm:text-6xl">{service.name}</h3><p className="mt-5 max-w-sm text-sm leading-7 text-foreground/60">{service.description || 'Spaces shaped by proportion, material and atmosphere.'}</p><span className="mt-7 inline-flex min-h-11 items-center gap-2 border-b border-foreground/35 pb-2 text-[10px] font-semibold uppercase tracking-[0.18em] transition-colors group-hover:border-gold group-hover:text-gold">Explore service <ArrowRight size={14} className="transition-transform group-hover:translate-x-1" aria-hidden="true" /></span></div><div className="order-1 aspect-[5/4] overflow-hidden bg-canvas-dark md:order-2"><img src={image} alt={service.name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" /></div></Link>
    </article>
  )
}
...
function ApproachSection() {
  ...
  return (
    <section className="border-y border-foreground/10 bg-canvas px-5 py-20 sm:px-8 sm:py-28 lg:px-12 lg:py-32"><div className="mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:gap-20"><div><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/55">03 — The Revamp approach</p><h2 className="mt-5 max-w-md font-serif text-5xl font-light leading-[0.9] sm:text-7xl">From vision<br /><span className="italic text-foreground/45">to reality.</span></h2><p className="mt-7 max-w-sm text-sm leading-7 text-foreground/60">We are not simply a designer who hands over drawings. We stay with the work until the final detail is in place.</p></div><div className="border-t border-foreground/15">{steps.map(([number, title, description]) => <div key={number} className="grid grid-cols-[2.5rem_0.75fr_1fr] gap-3 border-b border-foreground/15 py-5 sm:grid-cols-[3rem_0.65fr_1fr] sm:gap-6"><span className="text-[10px] tracking-[0.18em] text-foreground/45">{number}</span><h3 className="font-serif text-2xl font-light leading-none sm:text-3xl">{title}</h3><p className="text-xs leading-5 text-foreground/55 sm:text-sm sm:leading-6">{description}</p></div>)}</div></div></section>
  )
}
...
function MaterialSection() {
  ...
  return (
    <section className="bg-background px-5 py-20 sm:px-8 sm:py-28 lg:px-12 lg:py-36"><div className="mx-auto max-w-[1440px]"><div className="mb-10 max-w-3xl"><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/55">04 — Material dialogue</p><h2 className="mt-4 font-serif text-5xl font-light leading-[0.9] sm:text-7xl">Material is architecture.</h2></div><div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-4 md:items-start">{materials.map(([name, description], index) => <div key={name} className={cn('relative min-h-48 overflow-hidden border border-foreground/15 bg-canvas px-4 py-5 sm:min-h-64 sm:px-6 sm:py-7', index % 2 === 1 && 'mt-8 md:mt-12')}><div className="absolute inset-0 bg-gradient-to-br from-canvas-dark via-background to-canvas opacity-90" /><div className="relative flex h-full min-h-40 flex-col justify-between sm:min-h-56"><div className="flex items-start justify-between"><span className="text-[9px] uppercase tracking-[0.18em] text-foreground/45">0{index + 1}</span><span className="size-2 rounded-full bg-gold/70" /></div><div><h3 className="font-serif text-3xl font-light leading-none sm:text-4xl">{name}</h3><p className="mt-3 max-w-[11rem] text-[10px] leading-4 text-foreground/55">{description}</p></div></div></div>)}</div></div></section>
  )
}
  ...
  return (
    <>
      <main className="min-h-screen bg-background">
        <section className="relative overflow-hidden bg-canvas px-5 pb-16 pt-28 sm:px-8 sm:pb-24 sm:pt-36 lg:px-12 lg:pb-32"><div className="mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[1fr_0.72fr] lg:items-end"><div><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/55">The Revamp House</p><h1 className="mt-5 max-w-5xl font-serif text-[clamp(4rem,10vw,9rem)] font-light leading-[0.82] tracking-[-0.04em]">Designing the<br /><span className="italic text-foreground/45">way you live.</span></h1></div><div className="max-w-md border-t border-foreground/15 pt-6 lg:mb-1"><p className="text-sm leading-7 text-foreground/65 sm:text-base">From architecture and interiors to global sourcing and bespoke craftsmanship, we bring the entire design journey together under one roof.</p><div className="mt-7"><EditorialLink href="/book-consultation">Begin a conversation</EditorialLink></div></div></div><div className="pointer-events-none absolute -bottom-14 right-[8%] hidden font-serif text-[17rem] font-light leading-none text-foreground/[0.035] lg:block">01</div></section>

        <section className="border-y border-foreground/10 bg-background px-5 py-16 sm:px-8 sm:py-24 lg:px-12"><div className="mx-auto grid max-w-[1440px] gap-10 md:grid-cols-[0.5fr_1fr] md:items-start"><div><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/55">A complete design house</p></div><div><h2 className="max-w-4xl font-serif text-4xl font-light leading-[0.94] sm:text-6xl">We approach every project as a complete composition — architecture, interiors, objects, materials and craftsmanship considered together.</h2><p className="mt-6 max-w-xl text-sm leading-7 text-foreground/60">The result is not a collection of services, but a point of view carried consistently from vision to reality.</p></div></div></section>

        <section className="bg-background px-5 py-20 sm:px-8 sm:py-28 lg:px-12 lg:py-36"><div className="mx-auto max-w-[1440px]"><div className="mb-10 grid gap-8 border-b border-foreground/15 pb-8 md:grid-cols-[0.65fr_1.35fr] md:items-end"><div><p className="text-[10px] uppercase tracking-[0.28em] text-foreground/55">01 — The services</p><h2 className="mt-4 max-w-lg font-serif text-5xl font-light leading-[0.9] sm:text-7xl">Capabilities<br /><span className="italic text-foreground/45">with depth.</span></h2></div><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between sm:gap-8"><p className="max-w-lg text-sm leading-7 text-foreground/60">Each discipline is part of the same composition. Explore the chapters that make up the Revamp practice.</p><div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-foreground/50"><span>{totalResults}</span> {totalResults === 1 ? 'service' : 'services'}</div></div></div>

          <div className="sticky top-16 z-20 -mx-5 mb-12 border-y border-foreground/15 bg-background/92 px-5 py-4 backdrop-blur-md sm:-mx-8 sm:px-8 lg:-mx-12 lg:px-12"><div className="mx-auto flex max-w-[1440px] flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="relative max-w-md flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-foreground/45" aria-hidden="true" /><input type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Find a capability" className="h-10 w-full border-b border-foreground/25 bg-transparent pl-9 pr-9 text-xs uppercase tracking-[0.12em] text-foreground placeholder:text-foreground/40 focus:border-gold focus:outline-none" aria-label="Search services" />{searchQuery && <button type="button" onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-foreground/50 hover:text-gold" aria-label="Clear service search"><X size={15} /></button>}</div><div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none"><button type="button" onClick={() => setSelectedCategory('all')} className={cn('whitespace-nowrap border-b pb-1 text-[10px] uppercase tracking-[0.16em] transition-colors', selectedCategory === 'all' ? 'border-gold text-foreground' : 'border-transparent text-foreground/45 hover:text-foreground')}>All work</button>{categories.map((category) => <button type="button" key={category.id} onClick={() => setSelectedCategory(category.id)} className={cn('whitespace-nowrap border-b pb-1 text-[10px] uppercase tracking-[0.16em] transition-colors', selectedCategory === category.id ? 'border-gold text-foreground' : 'border-transparent text-foreground/45 hover:text-foreground')}>{category.name}</button>)}</div></div></div>

          {services.length ? <div>{services.map(({ service, category }, index) => <ServiceChapter key={`${category.slug}-${service.slug}`} service={service} category={category} index={index} />)}</div> : <div className="border border-dashed border-foreground/20 px-6 py-16 text-center"><p className="font-serif text-3xl font-light">No matching services found.</p><p className="mt-3 text-sm text-foreground/55">Try a different search or return to the complete practice.</p><button type="button" onClick={() => { setSearchQuery(''); setSelectedCategory('all') }} className="mt-6 border-b border-foreground/35 pb-2 text-[10px] uppercase tracking-[0.18em] hover:border-gold hover:text-gold">Clear filters</button></div>}
        </div></section>
