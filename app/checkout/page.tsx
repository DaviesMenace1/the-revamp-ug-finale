...
import { ArrowLeft, Check, CreditCard, Home, Loader2, Lock, MapPin, Pencil, Plus, ShieldCheck, ShoppingCart, Smartphone, Sparkles, Store } from '@/components/ui/luxury-icons'
...
export default function CheckoutPage() {
  ...
  const handlePayWithFlutterwave = async (event: React.FormEvent) => {
    ...
    try {
      ...
      throw new Error('Flutterwave did not return a usable payment state. Please try again.')
    } catch (error) {
  ...
  if (items.length === 0) {
    return <><SiteHeader /><main className="flex min-h-[70dvh] items-center justify-center px-6 pt-20"><div className="max-w-md text-center"><ShoppingCart className="mx-auto mb-5 size-10 text-primary" aria-hidden="true" /><h1 className="font-serif text-5xl tracking-tight">Nothing to check out yet</h1><p className="mt-4 text-sm leading-7 text-muted-foreground">Return to the collection to choose a considered piece for your space.</p><Link href="/collections" className="mt-8 inline-flex min-h-12 items-center justify-center rounded-md bg-primary px-6 text-xs font-medium uppercase tracking-[0.16em] text-primary-foreground">Return to shop</Link></div></main></>
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="bg-canvas px-4 pb-24 pt-28 sm:px-6 md:px-10 md:pt-36">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-7 max-w-3xl border-b border-border/70 pb-8"><p className="text-[10px] uppercase tracking-[0.3em] text-primary">Secure checkout</p><h1 className="mt-3 font-serif text-5xl tracking-tight md:text-6xl">Almost there.</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Thoughtful pieces. A more refined tomorrow.</p></div>
          ...
          <div className="mt-8 grid grid-cols-3 gap-2 border-b border-border/70 pb-6" aria-label="Checkout progress">
            {[['01', 'Information'], ['02', 'Shipping'], ['03', 'Payment'], ['04', 'Review']].map(([number, label], index) => {
              const step = index + 1
          ...
          <form onSubmit={handlePayWithFlutterwave} className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_380px] lg:gap-12">
            <div className="relative min-w-0 lg:block">
              ...
              {paymentInstruction && <div role="status" className="rounded-lg border border-amber-300/70 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950 dark:border-amber-400/40 dark:bg-amber-950/40 dark:text-amber-50"><p className="font-medium">Continue securely with Flutterwave</p><p className="mt-1">{paymentInstruction}</p><p className="mt-2 text-xs">Your order will be confirmed after Flutterwave verifies the payment.</p></div>}
              ...
              <Button type="button" onClick={() => setActiveStep(4)} disabled={hasMixedCurrencies || hasUnavailableItems} className="min-h-14 w-full rounded-md bg-primary text-xs font-semibold uppercase tracking-[0.16em] text-primary-foreground hover:bg-primary/90"><span className="flex items-center gap-2">Review order <ArrowLeft className="size-4 rotate-180" aria-hidden="true" /></span></Button>
              <div className="flex justify-between gap-3 lg:hidden"><Button type="button" variant="outline" onClick={() => setActiveStep(1)} className="min-h-11 rounded-md text-xs uppercase tracking-[0.12em]">Back to delivery</Button><span className="self-center text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Step 3 of 4</span></div>
              <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground"><ShieldCheck className="size-4 text-emerald-600" aria-hidden="true" /> Secure payment via Flutterwave</div>
              ...
              <section className={`${activeStep === 4 ? 'relative translate-x-0 opacity-100' : 'absolute inset-x-0 translate-x-6 opacity-0 pointer-events-none'} order-4 space-y-4 transition-[transform,opacity] duration-200 lg:static lg:translate-x-0 lg:opacity-100 lg:pointer-events-auto lg:order-none`}>
                <div className="rounded-lg border border-border/70 bg-card p-5 shadow-editorial sm:p-7"><div className="flex items-start justify-between gap-4 border-b border-border/70 pb-5"><div><p className="text-[10px] uppercase tracking-[0.24em] text-primary">04</p><h2 className="mt-2 font-serif text-3xl">Review & confirm</h2><p className="mt-2 text-xs leading-6 text-muted-foreground">Almost there. Please review your order before placing it.</p></div><Check className="mt-1 size-5 text-primary" aria-hidden="true" /></div><div className="mt-6 space-y-3">{items.map((item) => { const image = getProductImage(item); const unitPrice = itemUnitPrice(item); const currency = normalizeCurrency(item.currency ?? item.product?.currency); return <div key={`review-${item.cartItemId}`} className="flex items-center gap-3 border-b border-border/60 pb-3 last:border-0"><div className="relative size-14 shrink-0 overflow-hidden rounded-md bg-muted">{image ? <Image src={image} alt="" fill sizes="56px" className="object-cover" /> : <ShoppingCart className="m-auto size-5 text-primary" aria-hidden="true" />}</div><div className="min-w-0 flex-1"><p className="text-sm font-medium">{item.product?.name || 'Saved selection'}</p><p className="text-xs text-muted-foreground">Qty: {item.quantity}</p></div><p className="text-sm font-medium tabular-nums">{formatMoney(unitPrice * item.quantity, currency)}</p></div> })}</div><div className="mt-5 space-y-2 border-t border-border/70 pt-4 text-sm"><div className="flex justify-between gap-4 text-muted-foreground"><span>Subtotal</span><span>{formatMoney(cart?.subtotal || 0, checkoutCurrency)}</span></div><div className="flex justify-between gap-4 text-muted-foreground"><span>Delivery</span><span>{deliveryMethod === 'pickup_station' ? selectedPickupFee > 0 ? formatMoney(selectedPickupFee, checkoutCurrency) : 'Free pickup' : 'Confirmed after quotation'}</span></div>{promotionDiscountPreview > 0 && <div className="flex justify-between gap-4 text-primary"><span>Promotion</span><span>-{formatMoney(promotionDiscountPreview, checkoutCurrency)}</span></div>}<div className="flex justify-between gap-4 border-t border-border/70 pt-3 font-serif text-2xl text-foreground"><span>Total</span><span>{formatMoney(paymentTotal, checkoutCurrency)}</span></div></div><div className="mt-6 grid gap-2 sm:grid-cols-2"><Button type="button" variant="outline" onClick={() => setActiveStep(1)} className="min-h-12 rounded-md text-xs uppercase tracking-[0.12em]">Edit information</Button><Button type="button" variant="outline" onClick={() => setActiveStep(3)} className="min-h-12 rounded-md text-xs uppercase tracking-[0.12em]">Edit payment</Button></div>{errorMessage && <div role="alert" className="mt-5 rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm leading-6 text-destructive"><p className="font-medium">We could not continue</p><p className="mt-1">{errorMessage}</p></div>}{paymentInstruction && <div role="status" className="mt-5 rounded-lg border border-amber-300/70 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950"><p className="font-medium">Continue securely with Flutterwave</p><p className="mt-1">{paymentInstruction}</p></div>}{authorizationChallenge && <div className="mt-5 space-y-4 rounded-lg border border-primary/30 bg-primary/5 p-4"><label htmlFor="checkout-review-authorization-code" className="block text-sm font-medium text-foreground">{authorizationChallenge.authorizationType === 'pin' ? 'Sandbox card PIN' : 'Sandbox OTP'}</label><div className="flex flex-col gap-2 sm:flex-row"><Input id="checkout-review-authorization-code" type="password" inputMode="numeric" value={authorizationCode} onChange={(event) => setAuthorizationCode(event.target.value)} placeholder="Enter code" className="min-h-11 rounded-md bg-background" /><Button type="button" onClick={submitAuthorization} disabled={loading || !authorizationCode.trim()} className="min-h-11 rounded-md px-5">{loading ? 'Authorizing…' : 'Authorize payment'}</Button></div></div>}<Button type="submit" disabled={loading || hasMixedCurrencies || hasUnavailableItems} className="mt-6 min-h-14 w-full rounded-md bg-primary text-xs font-semibold uppercase tracking-[0.16em] text-primary-foreground hover:bg-primary/90">{loading ? <span className="flex items-center justify-center gap-2"><Loader2 className="size-4 animate-spin" aria-hidden="true" /> Preparing payment…</span> : <span className="flex items-center justify-center gap-2">{paymentMode === 'pay_now' ? <><Lock className="size-4" aria-hidden="true" /> Place order and pay securely</> : <><ShoppingCart className="size-4" aria-hidden="true" /> Place order, pay on delivery</>}</span>}</Button><p className="mt-3 text-center text-xs leading-5 text-muted-foreground">By placing this order, you agree to our <Link href="/refund-policy" className="font-medium text-primary underline underline-offset-4">cancellation and refund policy</Link>.</p></div>
              </section>
              </div>

            <aside className={`${activeStep === 2 ? 'relative translate-x-0 opacity-100' : 'absolute inset-x-0 translate-x-6 opacity-0 pointer-events-none'} order-2 h-fit overflow-hidden rounded-lg border border-border/70 bg-card shadow-editorial transition-[transform,opacity] duration-200 sm:p-0 lg:static lg:translate-x-0 lg:opacity-100 lg:pointer-events-auto lg:order-none lg:sticky lg:top-28`}>{items[0] && <div className="relative h-48 overflow-hidden bg-muted sm:h-56"><Image src={getProductImage(items[0])} alt="" fill sizes="(min-width: 1024px) 380px, 100vw" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-obsidian/80 via-obsidian/20 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-5 text-ivory"><p className="text-[10px] uppercase tracking-[0.24em] text-gold">The Revamp UG</p><p className="mt-2 font-serif text-2xl">More than products. A way of living.</p></div></div>}<div className="p-5 sm:p-6"><div className="flex items-end justify-between gap-4 border-b border-border/70 pb-5"><div><p className="text-[10px] uppercase tracking-[0.24em] text-primary">Your selection</p><h2 className="mt-2 font-serif text-3xl">Order summary</h2></div><span className="text-xs text-muted-foreground">{items.length} {items.length === 1 ? 'piece' : 'pieces'}</span></div>
