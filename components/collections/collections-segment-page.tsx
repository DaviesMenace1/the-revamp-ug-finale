        ...
        return (
          <>
            ...
            <main className="min-h-screen bg-canvas text-obsidian">
              <section className="relative overflow-hidden bg-obsidian text-ivory"><div className="absolute inset-0"><Image src={subcategories[0]?.image || '/prototype/hero-natural-light.jpg'} alt={categoryName} fill className="object-cover opacity-70" /><div className="absolute inset-0 bg-gradient-to-r from-black/75 via-black/30 to-transparent" /></div><div className="relative mx-auto flex min-h-[30rem] max-w-[1440px] flex-col justify-end px-5 pb-10 pt-28 sm:px-8 sm:pb-14 lg:px-12"><Link href="/collections" className="mb-auto inline-flex min-h-10 items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-ivory/70 hover:text-white"><ArrowLeft className="size-3" /> Collections</Link><div className="max-w-xl"><p className="text-[10px] uppercase tracking-[0.28em] text-ivory/60">{categoryName}</p><h1 className="mt-4 font-serif text-6xl font-light leading-[0.86] sm:text-8xl">{categoryName === 'Seating' ? <>Chairs, sofas<br />and all the ways you sit well.</> : <>{categoryName}<br />for refined living.</>}</h1><p className="mt-5 max-w-md text-sm leading-6 text-ivory/75">{categoryName === 'Seating' ? 'From sculptural lounge chairs to timeless sofas, this collection brings together comfort, craftsmanship, and iconic design.' : `A considered edit of ${categoryName.toLowerCase()} for spaces with a point of view.`}</p></div></div></section>
              <div className="mx-auto max-w-[1440px] px-5 py-12 sm:px-8 sm:py-16 lg:px-12"><div className="mb-8 flex items-end justify-between border-b border-border pb-5"><div><p className="text-[10px] uppercase tracking-[0.24em] text-muted-foreground">Collections / {categoryName}</p><h2 className="mt-3 font-serif text-4xl sm:text-5xl">Explore the edit.</h2></div><span className="text-xs text-muted-foreground">{subcategories.length} edits</span></div><div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3">{subcategories.map((subcategory) => <Link key={subcategory.slug} href={`/collections/${segments[0]}/${subcategory.slug}`} className="group"><div className="relative aspect-[4/5] overflow-hidden bg-muted">{subcategory.image ? <Image src={subcategory.image} alt={subcategory.name} fill sizes="(max-width: 640px) 50vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" /> : <div className="absolute inset-0 bg-muted" />}<div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-4 pt-16 text-white"><p className="text-[9px] uppercase tracking-[0.18em] text-white/70">{subcategory.productCount} pieces</p><h2 className="mt-2 font-serif text-3xl leading-none">{subcategory.name}</h2></div></div></Link>)}</div></div>
            </main>
            <SiteFooter />
          </>
        )
  ...
  if (!product && segments.length === 2) {
    ...
    if (subcategoryProducts.length > 0) {
      ...
      return (
        <>
          <SiteHeader />
          <main className="min-h-screen bg-canvas px-5 pb-24 pt-20 text-obsidian sm:px-8 md:px-12 md:pt-28"><div className="mx-auto max-w-[1440px]"><nav className="flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-muted-foreground"><Link href="/collections">Collections</Link><span>/</span><Link href={`/collections/${segments[0]}`}>{categoryName}</Link><span>/</span><span className="text-foreground">{subcategoryName}</span></nav><header className="mt-12 flex flex-col justify-between gap-6 border-b border-border pb-8 md:flex-row md:items-end"><div><p className="text-[10px] uppercase tracking-[0.28em] text-muted-foreground">{categoryName}</p><h1 className="mt-4 font-serif text-6xl font-light leading-[0.86] sm:text-8xl">{subcategoryName}</h1><p className="mt-5 max-w-md text-sm leading-6 text-muted-foreground">Distinctive pieces for refined interiors, featuring a curated selection of global brands and bespoke designs.</p></div><p className="text-xs text-muted-foreground">{subcategoryProducts.length} products</p></header><div className="pt-8"><CollectionProductGrid products={subcategoryProducts as any} /></div></div></main><SiteFooter />
        </>
      )
    }
  }
