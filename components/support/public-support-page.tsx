      ...
      <main className="min-h-screen bg-background">
        <section className="relative overflow-hidden bg-obsidian px-5 pb-14 pt-36 text-ivory sm:px-8 sm:pb-20 md:pt-44 lg:px-16">
          ...
          <div className="relative mx-auto grid max-w-[1280px] gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <div className="flex items-center gap-3 border-l border-gold/45 pl-5 text-sm text-ivory/65"><LuxuryLifeBuoy className="size-5 shrink-0 text-gold" aria-hidden="true" /><span>Human support<br />Kampala · East Africa</span></div>
          </div>
        </section>
          ...
          <div className="mx-auto grid max-w-[1180px] gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-12">
            <aside className="space-y-8">
              ...
              <div className="grid gap-3 border-t border-border/70 pt-6">
                <a href={`mailto:${siteContact.primaryEmail}`} className="flex min-h-12 items-center gap-3 text-sm text-foreground transition-colors hover:text-primary"><LuxuryMail className="size-4 shrink-0 text-gold" aria-hidden="true" />{siteContact.primaryEmail}</a>
                <a href={siteContact.phoneHref} className="flex min-h-12 items-center gap-3 text-sm text-foreground transition-colors hover:text-primary"><LuxuryPhone className="size-4 shrink-0 text-gold" aria-hidden="true" />{siteContact.phoneDisplay}</a>
                <a href="https://wa.me/256783476807" target="_blank" rel="noreferrer" className="flex min-h-12 items-center gap-3 text-sm text-foreground transition-colors hover:text-primary"><LuxuryMessageSquare className="size-4 shrink-0 text-gold" aria-hidden="true" />WhatsApp the studio<LuxuryArrowUpRight className="ml-auto size-4 shrink-0" aria-hidden="true" /></a>
              </div>
              ...
              <div className="rounded-xl border border-border/70 bg-card p-5 shadow-soft">
                ...
                <Link href="/sign-in?redirect_url=%2Fclient%2Ftickets" className="mt-4 inline-flex min-h-11 items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-primary hover:underline">View my tickets<LuxuryArrowUpRight className="size-4" aria-hidden="true" /></Link>
              </div>
            </aside>
            ...
            <div className="rounded-2xl border border-border/70 bg-card p-5 shadow-soft sm:p-8">
              {submitted ? (
                <div className="flex min-h-[25rem] flex-col items-center justify-center text-center">
                  <span className="flex size-16 items-center justify-center rounded-full bg-gold/15 text-primary"><LuxuryCheckCircle2 className="size-8" aria-hidden="true" /></span>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <Button type="submit" disabled={isPending} className="min-h-12 w-full rounded-none text-xs uppercase tracking-[0.18em]">{isPending ? 'Sending request…' : 'Send support request'}<LuxuryArrowUpRight className="ml-2 size-4" aria-hidden="true" /></Button>
